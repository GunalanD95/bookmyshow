...
def seed_data_function():
    ...
    with Session() as session:
        ...
        movie1 = Movie(
            name="Avengers", auditorium_id=auditorium1.id, languages=Languages.ENGLISH
        )
        movie1.actors.extend([actor1, actor2])

        movie2 = Movie(
            name="Thor", auditorium_id=auditorium2.id, languages=Languages.ENGLISH
        )
        ...
        # Add shows
        show1 = Show(
            auditorium_id=auditorium1.id,
            movie_id=movie1.id,
            start_time="2022-06-01 10:00:00",
            end_time="2022-06-01 12:00:00",
            show_features={"show_type": "2D"},
        )
        show2 = Show(
            auditorium_id=auditorium2.id,
            movie_id=movie2.id,
            start_time="2022-06-01 10:00:00",
            end_time="2022-06-01 12:00:00",
            show_features={"show_type": "2D"},
        ...
        seat_type_1 = ShowSeatType(
            show_id=show2.id, seat_type=SeatType.BRONZE, price=100
        )
        ...
        for i in range(1, 11):
            for j in range(1, 11):
                seat1 = Seat(
                    name=f"IMAX Screen Seat {i,j}",
                    auditorium_id=auditorium1.id,
                    seat_num=i * 10 + j,
                    row=i,
                    col=j,
                    seat_status=SeatStatusEnum.AVAILABLE,
                )
                seat2 = Seat(
                    name=f"PVR Screen  Seat {i,j}",
                    auditorium_id=auditorium2.id,
                    seat_num=i * 10 + j,
                    row=i,
                    col=j,
                ...
                show_seat = ShowSeat(
                    show_id=show1.id,
                    seat_id=seat1.id,
                    show_seat_type=seat_type.id,
                    status=SeatStatus.AVAILABLE,
                )
                show_seat_1 = ShowSeat(
                    show_id=show2.id,
                    seat_id=seat2.id,
                    show_seat_type=seat_type_1.id,
                    status=SeatStatus.AVAILABLE,
                )
